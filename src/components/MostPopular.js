...
const MostPopular = ({ blogs }) => {
    ...
    return (
        <div>
            ...
            {
                blogs?.map((item) => {
                    return <div className='row pb-3' key={item.id} onClick={() => navigate(`/detail/${item.id}`)}>
                        ...
                        <div className='col-7 padding'>
                            ...
                            <div className='text-start most-popular-font'>{item.timestamp.toDate().toDateString()}</div>
                        </div>
                    </div>
                })
            }
